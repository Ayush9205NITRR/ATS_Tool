    ...
    <div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
        ...
        <div className="lg:col-span-3 space-y-5">
          ...
          {/* Interview Notes — collapsible per section */}
          <div>
                        ...
                        {entries.map((e, i) => {
                          ...
                          return (
                            <div key={i} className="bg-gray-50 rounded-lg px-3 py-2.5 group/note">
                              {isEditing ? (
                                <div className="space-y-2">
                                  <textarea autoFocus rows={3} value={editingNote.text}
...
// ── Collapsible Section for Interview Notes ───────────────────
function CollapsibleSection({
  label, count, defaultOpen, borderTop, children
}: {
  label: string
  count: number
  defaultOpen: boolean
  ...
  return (
    <div className={borderTop ? 'border-t border-gray-100' : ''}>
      {/* Header — click to toggle */}
      <button
        type="button"
        onClick={() => setOpen(o => !o)}
        className="w-full flex items-center gap-2 px-4 py-3 hover:bg-gray-50/60 transition-colors text-left"
      >
        <ChevronDown className={`w-3.5 h-3.5 text-gray-400 transition-transform flex-shrink-0 ${open ? '' : '-rotate-90'}`}/>
        <span className={`text-xs font-semibold uppercase tracking-wide flex-1 ${count > 0 ? 'text-gray-700' : 'text-gray-400'}`}>
          {label}
        </span>
        {count > 0 && (
          <span className="text-xs bg-slate-100 text-slate-600 px-1.5 py-0.5 rounded-full font-medium flex-shrink-0">
            {count}
          </span>
        )}
      </button>
