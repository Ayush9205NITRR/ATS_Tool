// ============================================================
// SETTINGS PAGE — user management (Super Admin only)
// ============================================================
...
const schema = z.object({
  ...
  role: z.enum(['super_admin','admin','interviewer']),
})
...
export function SettingsPage() {
  ...
  const createUser = useMutation({
    mutationFn: async (d: FormData) => {
      // Create auth user via Supabase admin (requires service role on server side)
      // For now, create directly in users table — in production use Edge Function
        ...
        <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
          {users.map((u, i) => (
            <div key={u.id} className={`flex items-center gap-4 px-5 py-4 ${i > 0 ? 'border-t border-gray-100' : ''}`}>
              ...
              <span className={`text-xs font-medium px-2.5 py-1 rounded-full ${ROLE_COLOUR[u.role]}`}>
                {labelOf(u.role)}
              </span>
              <Button
      ...
      <Modal open={showModal} onClose={() => setShowModal(false)} title="Add Team Member" size="sm">
        <form onSubmit={handleSubmit((d) => createUser.mutate(d))} className="space-y-4">
          ...
          <div>
            ...
            <input {...register('email')} type="email" placeholder="priya@company.com" className={inputCls} />
