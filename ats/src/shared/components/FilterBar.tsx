...
const OPS: Record<string, { op: FilterOp; label: string }[]> = {
  ...
  multi:  [{ op: 'is_any_of', label: 'is any of' }, { op: 'is_none_of', label: 'is none of' }, { op: 'is', label: 'is exactly' }],
}

const SOURCES = ['platform', 'agency', 'college']

export function applyFilters(candidates: any[], filters: ActiveFilter[], jobs: any[], mode: FilterMode = 'and'): any[] {
  if (!filters.length) return candidates
  const match = (c: any, f: ActiveFilter) => {
    const raw = f.field === 'job_id'
      ? (c.job_id ?? c.job?.id ?? '').toString().toLowerCase().trim()
      : (c[f.field] ?? '').toString().toLowerCase().trim()
...
interface Props {
  filters: ActiveFilter[]; onChange: (f: ActiveFilter[]) => void
  jobs: { id: string; title: string }[]
  mode: FilterMode; onModeChange: (m: FilterMode) => void
}

export function FilterBar({ filters, onChange, jobs, mode, onModeChange }: Props) {
          ...
          <div className="flex items-center gap-1 bg-gray-100 p-0.5 rounded-lg">
            {(['and','or'] as FilterMode[]).map(m => (
              <button key={m} onClick={() => onModeChange(m)}
                className={`px-2.5 py-1 rounded-md text-xs font-semibold transition-all uppercase ${mode === m ? 'bg-white shadow text-gray-800' : 'text-gray-400 hover:text-gray-600'}`}>
                {m}
              </button>
            ))}
          </div>
            ...
            <div key={f.id}>
              ...
              <div className="flex items-center gap-2">
                <select value={f.field}
                  onChange={e => { const nd = FILTER_FIELDS.find(ff => ff.key === e.target.value); upd(f.id, { field: e.target.value, op: (OPS[nd?.type??'text'])[0].op, value: '', values: [] }) }}
