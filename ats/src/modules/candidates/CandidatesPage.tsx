...
export function CandidatesPage() {
  ...
  const createMagicLink = useMutation({
    mutationFn: async (cid: string) => {
      const { data, error } = await supabase.from('magic_review_links')
        .insert({ candidate_id: cid, created_by: user!.id }).select().single()
      if (error) throw error
      await navigator.clipboard.writeText(`${window.location.origin}/review/${data.id}`)
    },
  })
                    ...
                    <tr key={c.id} className={`transition-colors ${isSel ? 'bg-blue-50/50' : 'hover:bg-gray-50/40'} ${c.archived_at ? 'opacity-50' : ''}`}>
                      ...
                      <td className="px-3 py-2.5">
                        <div className="flex items-center gap-1 justify-end">
                          {canAssign && (
                            <TipBtn colour="blue" tip="Copy interview link"
                              onClick={async () => { try { await createMagicLink.mutateAsync(c.id); alert('🔗 Copied!') } catch { alert('Failed') } }}>
                              🔗
                            </TipBtn>
                          )}
