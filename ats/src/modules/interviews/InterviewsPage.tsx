// ============================================================
// INTERVIEWS PAGE — shows only candidates assigned to this interviewer
// ============================================================
    ...
    <div>
      ...
      {isLoading ? (
        ...
        <EmptyState
          ...
          description="You'll see candidates here once an admin assigns them to you for interview."
        />
      ) : (
      ...
      <Modal open={!!selected} onClose={() => setSelected(null)} title={`Feedback — ${selected?.full_name}`} size="md">
        <form onSubmit={handleSubmit((d) => submitFeedback.mutate(d))} className="space-y-5">
            ...
            <div key={field}>
              ...
              <div className="flex gap-2">
                {[1,2,3,4,5].map((n) => (
                  <button
                    key={n} type="button"
                    onClick={() => setValue(field, n)}
                    className={`w-9 h-9 rounded-lg flex items-center justify-center transition-colors ${
                      watch(field) >= n ? 'bg-amber-400 text-white' : 'bg-gray-100 text-gray-400 hover:bg-amber-100'
                    }`}
                  >
          ...
          <div>
            ...
            <textarea {...register('strengths')} rows={2} placeholder="What stood out…" className={inputCls} />
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">Concerns</label>
            <textarea {...register('concerns')} rows={2} placeholder="Any reservations…" className={inputCls} />
          </div>
          ...
          <div>
            ...
            <div className="flex gap-2 flex-wrap">
              {RECS.map((r) => (
                <button
                  ...
                  onClick={() => setValue('recommendation', r.value)}
                  className={`px-3 py-1.5 rounded-lg text-sm font-medium transition-all border-2 ${
                    rec === r.value ? r.colour + ' border-transparent' : 'bg-white text-gray-600 border-gray-200 hover:border-gray-300'
                  }`}
                >
                  {r.label}
                </button>
            ...
            {errors.recommendation && <p className="mt-1 text-xs text-red-600">Select a recommendation</p>}
          </div>
