...
export function ResumeUploadPage() {
  ...
  return (
    <div>
      <PageHeader
        title="Upload Resume"
        subtitle="Parse resumes from URLs to auto-extract candidate information."
      />
      ...
      <div className="flex gap-3 mb-6">
        <ModeCard
          ...
          title="Single Resume URL"
          description="Fetch & parse one resume"
          onClick={() => setParams({ mode: 'single' })}
        />
        <ModeCard
          active={mode === 'bulk'}
          icon={<FileSpreadsheet className="w-5 h-5" />}
          title="Bulk Resume URLs"
          description="Upload CSV with multiple URLs"
          onClick={() => setParams({ mode: 'bulk' })}
        />
      </div>

      <div className="bg-white rounded-xl border border-gray-200 p-6">
        {mode === 'single' ? <SingleResumeUpload /> : <BulkResumeUpload />}
      </div>
    </div>
  )
}
