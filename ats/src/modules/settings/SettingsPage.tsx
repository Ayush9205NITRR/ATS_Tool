...
export function SettingsPage() {
  ...
  return (
    <div>
      <PageHeader title="Settings" subtitle="Manage team members and access"
        ...
        <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
          {users.map((u, i) => (
            <div key={u.id} className={`flex items-center gap-4 px-5 py-4 ${i > 0 ? 'border-t border-gray-100' : ''}`}>
              ...
              <span className={`text-xs font-medium px-2.5 py-1 rounded-full ${ROLE_COLOUR[u.role]}`}>{labelOf(u.role)}</span>
