...
export function CandidatesPage() {
  ...
  const toggleCol = useCallback((key: string) =>
    setVisibleCols(p => { const n = new Set(p); n.has(key) ? n.delete(key) : n.add(key); return n }), [])

  const toggleSel = useCallback((id: string) =>
    setSelectedIds(p => { const n = new Set(p); n.has(id) ? n.delete(id) : n.add(id); return n }), [])

  const toggleAll = useCallback(() =>
    setSelectedIds(s => s.size === displayed.length
      ? new Set()
      : new Set(displayed.map((c: any) => c.id))
    ), [displayed])

  const getName = useCallback((list: any[], id: string | null) => {
    if (!id) return null
    const i = list.find(u => u.id === id)
    return i?.full_name ?? i?.title ?? null
  }, [])

  const show = useCallback((key: string) => visibleCols.has(key), [visibleCols])

  const colCols = useMemo(() =>
    [...ALL_COLUMNS, ...(customFields as any[]).map(f => ({ key: `cf_${f.field_name}`, label: f.field_label }))],
    [customFields]
  )
                      ...
                      {bulkField && bulkField !== '__delete__' && (
                        <div className="mt-2 pt-2 border-t border-gray-100">
                          <p className="text-xs text-gray-500 mb-1">
                            {bulkField === 'current_stage' ? 'Set Stage'
                              : bulkField === 'job_id' ? 'Assign Job'
                              : bulkField === 'assigned_interviewers' ? 'Add Interviewer(s)'
                              : 'Add HR Owner(s)'}
                          </p>
                          {['assigned_interviewers','assigned_hr_owners'].includes(bulkField) && (
                            <p className="text-xs text-blue-600 mb-1.5">Will be added to existing assignments</p>
                          )}
                          <select autoFocus defaultValue=""
                            onChange={e => {
                              if (!e.target.value) return
                              const isArr = ['assigned_interviewers','assigned_hr_owners'].includes(bulkField)
                              bulkUpdate.mutate({ field: bulkField, value: isArr ? [e.target.value] : e.target.value })
                            }}
